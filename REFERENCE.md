...
### `edge { source, target, path ? [ ], mode ? "merge", adapt ? null, annotations ? { } }` → `<edge>`

Builds the canonical edge record `{ source; target; path; mode; adapt; annotations; }` (§4.1).
...
is `{ target; path; source; mode; annotations; }` with `source`/`target` carrying identity arms only.

### `renderTrace E` → `string`

Display rendering of a trace, one line per entry:
`<targetKey> | <pathKey> | <sourceKey> | <mode>`. The frozen §4.4 renderings:
